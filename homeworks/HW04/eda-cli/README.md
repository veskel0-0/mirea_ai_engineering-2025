...
- вызывает функции из `eda_cli.core`:
  ...
  - `missing_table`,
  - `compute_quality_flags`;
- возвращает флаги качества датасета в формате FlagResponse.
...
Ответ будет содержать список значений флагов:

- `too_few_rows` - слишком мало строк;
- `too_many_columns` - слишком много столбцов;
- `max_missing_share` - максимальная доля пропущенных значений в любом столбце;
- `too_many_missing` - слишком много пропусков
- `has_many_zero_values` - слишком много нулевых значений;
- `has_high_cardinality_categoricals` - слишком много категориальных столбцов;
- `quality_score` - интегральный скор качества.
