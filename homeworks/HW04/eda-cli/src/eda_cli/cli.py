...
def report(
    ...
    min_missing_share: float = typer.Option(0.5, help="Порог доли пропусков для выделения проблемных колонок (от 0.0 до 1.0)."),
) -> None:
    """
    ...
    top_cats = top_categories(df, max_columns=100, top_k=top_k_categories)  # ← передаём top_k
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Макс. колонок в гистограммах: **{max_hist_columns}**\n")
        ...
        if "has_many_zero_values" in quality_flags:
            f.write(f"- Есть колонки с множеством нулей: **{quality_flags['has_many_zero_values']}**\n")
        if "has_suspicious_id_duplicates" in quality_flags:
            f.write(f"- Подозрение на дубликаты в ID: **{quality_flags['has_suspicious_id_duplicates']}**\n")
        f.write("\n")
        ...
        if missing_df.empty:
            ...
            if not problematic.empty:
                f.write("### Проблемные колонки (доля пропусков выше порога):\n")
                for col in problematic.index:
                    share = problematic.loc[col, "missing_share"]
        ...
        if not top_cats:
            ...
            f.write(f"Для каждой категории показано top-{top_k_categories} значений.\n")
