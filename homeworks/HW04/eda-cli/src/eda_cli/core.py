...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, df: pd.DataFrame) -> Dict[str, Any]:
    ...
    if len(numeric_cols) == 0:
        ...
        zero_shares = (df[numeric_cols] == 0).mean()
        flags["has_many_zero_values"] = bool((zero_shares >= 0.3).any())

    
    categorical_uniques = [col.unique for col in summary.columns if not col.is_numeric]
    has_high_cardinality = False
    if categorical_uniques:
        max_unique = max(categorical_uniques)
        has_high_cardinality = max_unique > 0.4 * summary.n_rows
